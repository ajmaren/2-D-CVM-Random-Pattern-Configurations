...
#   To adjust the ratio, tweak the parameter x-ratio in main. It is passed into:
#   -  and
#   -  
...
#   All that it does is randomly populate a 2-D CVM grid, with a random generation of (presumably) 
#   equal numbers of x1 and x2 nodes. 
#   Fairly often, this random generation does not yield x1 = x2, so it does an "adjustment step." 
#   During this adjustment (currently set to a max of three steps), it will look for a node to flip
#   to alter the x1 / x2 ratio. If it succeeds in finding a "flippable" node (one that brings the 
#   x1 and x2 proportions closer to x1 = x2), then it keeps the flip. 
#   At the end, it computes the thermodynamic variables. 
#
# Dec. 17, 2017 (cont.): h is not really used in this program; it is incremented and passed along into 
#   the computeThermodynamicVariables, so it influences the different interaction enthalpy values, but 
#   is not really important in this code - the incrementing of h is a leftover from previous code,
#   and is kept here simple so that I have a loop for testing various iterations of randomly-generating
#   (and then adjusting) 2-D grids. I just want to be sure that the adjustment steps work ok, which is 
#   the purpose of this entire code.  
...
#    Inputs:    TBD
#    Return:    TBD
# Note: This procedure never got written; it's still on the to-do list. 
#
####################################################################################################
####################################################################################################

def createEasyToReadGrid (arraySizeList, unitArray):

    print ' creating an easy-to-read grid'
    easyGridArray = ['-', 'X', '-', 'X']
    return (easyGridArray)
...
def initializeMatrix (arraySizeList, h, x1TargetVal, maxXDif, i, numTrials):
...
def computeConfigAndThermVars(arraySizeList, h, x1TargetVal, maxXDif, numTrials, jrange, maxRange):
    ...
    for i in range (0, numTrials, 1):        
        ...
        configVarsList = list ()  # empty list
        unitArray    = initializeMatrix (arraySizeList, h, x1TargetVal, maxXDif, i, numTrials)        
# debug print: 
#        print unitArray  
...
def printX1ProbAndAnalyticResults (x1ValsArray, y2ValsArray, z1ValsArray, negSValsArray, x1TotalSteps, 
    ...
    negSAnalyticValsArray   = np.zeros(x1TotalSteps, dtype=np.float)    
    ...
    print ' '
    print ' These results are for investigation of the randomly-generated'
    print '   (and modified) unit array versus the analytic results,'
    print '   specifically for the case where h = 0.0. '
    print ' This means that we can compute the probabilistic expectations '
    print '   for the configuration variables using the initial x1 specification.'
    print ' This serves as a V&V step for comparing the probabilistically-'
    print '   generated unitArray (which is then adjusted so that '
    print '   x1Actual = x1Target) actually has the correct probabilistic'
    print '   (within the tolerance specified as maxXDif; a parameter in **main**). '
    print ' ' 
    print ' The parameter j below is the iteration count through various x1 trials.'
    print ' ' 
    print '   j     x1   x1Target   y2       z1      negS   negSAnalytic'

    for j in range (0, x1TotalSteps, step):
        x1TargetVal = x1TargetVal - x1TargetIncrement 
        x1TargetValsArray[j] = x1TargetVal
        # Compute the corollary analytic values
        analyticVarsList = computeAnalyticConfigAndThermVars (x1TargetVal)

        y2AnalyticValsArray[j]   = analyticVarsList[0]
        z1AnalyticValsArray[j]   = analyticVarsList[1]
        negSAnalyticValsArray[j] = analyticVarsList[3]
                
        print '  ', j, '  %.4f' % (x1ValsArray[j]), '  %.2f' % (x1TargetVal), '  %.4f' % (y2ValsArray[j]) , \
        '  %.4f' % (z1ValsArray[j]) , '  %.4f' % (negSValsArray[j]), '  %.4f' % (negSAnalyticValsArray[j])
    
    # Plot the results from that FOR loop (multiple tests of a random grid for a given x1 value)          
    #    print '     and the free energy is in red, also shifted by 0.6.'       
    
    pylab.figure(1)
    pylab.plot (x1TargetValsArray,negSValsArray)
    pylab.plot (x1TargetValsArray, negSAnalyticValsArray, 'm')            
    pylab.plot (x1TargetValsArray, y2ValsArray-0.8, 'g')
    pylab.plot (x1TargetValsArray, y2AnalyticValsArray-0.8, 'r')
    pylab.plot (x1TargetValsArray, z1ValsArray-0.5, 'c')
    pylab.plot (x1TargetValsArray, z1AnalyticValsArray-0.5, 'k')
           
    # #   pylab.plot (xArray,fEps1Array,'m')
    # #   pylab.plot (xArray,fEnergyArray,'r')
    print ' ' 
    print ' ' 
    print ' Plot of config. variables and the negative entropy vs. x1'
    print ' '
    print ' This plot contains the same information as shown in the previous table.'
    print ' '
    print ' The value for h is set to 1.0 for these results.'
    ...
    print '    and the analytic negative entropy is in maroon. '
    print '  The probabilisticy2 values (minus 0.8) are in green,' 
    print '    and the analytic y2 (minus 0.8) are in red. '
    print '  The probabilistic z1 values (minus 0.5) are in cyan, '
    print '    and the analytic z1 values (minus 0.5) are in black.'
    pylab.show()  
...
def printEntropyDetailedResults (x1ValsArray, y1ValsArray, y2ValsArray, y3ValsArray, w1ValsArray, w2ValsArray, w3ValsArray, 
    ...
    print ' ' 
    print ' in printResults'
    print '  hArray[0] = ', hArray[0]
        
    print ' Final results, for the case where the starting x1 value is  %.4f' % (x1StartingVal - x1TargetIncrement)
...
#        if not debugPrintOff:
    ...
    hEqualsOneSuccessBool = 0
    if abs(h-1.0) < 0.001: hEqualsOneSuccessBool = 1
    if hEqualsOneSuccessBool == 1:
